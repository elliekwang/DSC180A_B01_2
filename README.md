...
## Conclusion
From October-December 2025, our goal was to create a strong model that could accurately predict the categories people's spendings would be attributed to. After applying Regex processing and thoroughly testing a variety of models, we discovered DistilBERT to be the most reliable, offering an accuracy of 95%. <br>
From January 2026 to present, we have added onto our previous findings to build a model that uses consumer spending categories to help predict whether the respective consumer is delinquent. We have sampled delinquent and non-deliquent consumers, aiming to explore how their balances change over time. We have also feature engineered numerous features, creating feature plots and implementing model testing to evaluate our most significant features to keep. Namely, we focused on the four models: Logistic Regression, Random Forest, XGBoost, and LightGBM, using hyperparameter tuning using grid search and optimizing our results with a threshold. We evaluated the pros and cons of various metrics, particularly AUC, precision, and recall, when selecting our "best" model. We further improved our model by restricting our data to certain consumers (e.g. Those without an account or transactions) and classified each consumer as either 0 (non-delinquent) or 1 (delinquent). Using the predict_proba() method, we were able to cast a probability estimating the likelihood of defaulting for each consumer, which we scaled into a numeric value between 300 and 850 to match the scale of true credit scores. 